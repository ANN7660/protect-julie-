...
async def on_member_join(member: discord.Member):
    ...
    # keep existing welcome behavior: if there is another on_member_join defined earlier, this will run too

# --- createvoc command to create the trigger voice channel ---
...
# --- Auto temporary voice channels ---
@bot.event
async def on_voice_state_update(member, before, after):
    try:
        # If user joins the trigger channel named "Créer un vocal" (case-insensitive)
        if after is not None and after.channel is not None and after.channel.name.lower() == "créer un vocal":
            category = after.channel.category
            guild = after.channel.guild
            ...
            # Background task to delete channel when empty
            async def monitor_and_delete(vc):
                await asyncio.sleep(8)
                while True:
                    # fetch fresh channel object
                    ch = guild.get_channel(vc.id)
                    if ch is None:
                        break
                    if len(ch.members) == 0:
                        try:
            ...
            bot.loop.create_task(monitor_and_delete(new_vc))

        # Optional: if user leaves a temporary vc and it's empty, cleanup is handled by monitor
    except Exception as e:
        print(f"❌ Erreur système voc: {e}")
